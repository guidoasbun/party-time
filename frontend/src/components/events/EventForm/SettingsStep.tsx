...
export function SettingsStep({
  ...
  const getFieldError = (fieldName: string): string | undefined => {
    return (
      errors?.[fieldName] ||
      formErrors[fieldName as keyof typeof formErrors]?.message
    );
  };
            ...
            <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800 rounded-md">
              <div className="flex items-start space-x-2">
                ...
                <div className="text-sm text-blue-800 dark:text-blue-200">
                  <strong>Public Event:</strong> Anyone with the link can view
                  event details and RSVP if enabled. Guest list and contact
          ...
          <div className="space-y-4">
            ...
            {maxGuests && (
              <div className="text-sm text-muted-foreground">
                This event can accommodate up to <strong>{maxGuests}</strong>{" "}
            ...
            {budgetTotal !== undefined && (
              <div className="space-y-2">
                ...
                {maxGuests && maxGuests > 0 && budgetTotal > 0 && (
                  <div className="text-sm text-muted-foreground">
                    Budget per guest:{" "}
                    <strong>{formatCurrency(budgetTotal / maxGuests)}</strong>
                  </div>
                )}
              </div>
            )}
              ...
              <div className="space-y-3">
                ...
                <div className="text-sm text-muted-foreground">
                  {status === EventStatus.DRAFT &&
                    "You can continue editing and save as draft to come back later."}
                  {status === EventStatus.PLANNING &&
                    "Event is in active planning phase. You can invite guests and manage details."}
                  {status === EventStatus.CONFIRMED &&
                    "Event is confirmed and ready. Guests can RSVP and view final details."}
                </div>
              </div>
            )}
          />

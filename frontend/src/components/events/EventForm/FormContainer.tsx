...
export function FormContainer({
  ...
  const goToStep = React.useCallback((targetStepIndex: number) => {
    if (canNavigateToStep(targetStepIndex, currentStepIndex, completedSteps)) {
  ...
  }, [isFirstStep, isLastStep, isStepValid, goToPreviousStep, goToNextStep, formData, formId])
        ...
        <div className="mb-8">
          ...
          <div className="flex justify-between">
            {FORM_STEPS.map((step, index) => {
              ...
              const canAccess = canNavigateToStep(index, currentStepIndex, completedSteps)
...
// Helper functions
function getStepData(formData: Partial<EventCreateFormData>, stepName: FormStepName): unknown {
  switch (stepName) {
    case 'basicInfo':
      return {
        name: formData.name,
...
function isFieldInCurrentStep(fieldPath: string, stepName: FormStepName): boolean {
  const stepFields: Record<FormStepName, string[]> = {
    ...
    notificationSettings: ['notification_settings.send_invitations', 'notification_settings.reminder_schedule', 'notification_settings.auto_reminders'],
  }

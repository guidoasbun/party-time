  ...
  const skeletons = Array.from({ length: count }, (_, i) => (
    <div
      ...
      className={cn(
        ...
        viewMode === "grid"
          ? "h-64 sm:h-72 p-4 sm:p-6"
          : "h-24 sm:h-28 p-3 sm:p-4",
        animated && [
          "skeleton-enhanced",
...
export function EventList({
  ...
  return (
    <div className={cn("space-y-3 sm:space-y-4", className)}>
      ...
      {/* Loading state */}
      {isLoading ? (
        <EventListSkeleton viewMode={viewMode} animated={enableAnimations} />
      ) : events.length === 0 ? (
        /* Empty state */
        <EventEmptyState
        ...
        <>
          ...
          <div
            className={cn(
              ...
              viewMode === "grid"
                ? "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"
                : "grid-cols-1",
              enableAnimations && ["view-transition", "will-change-transform"]
            )}
            style={
              enableAnimations
                ? {
          ...
          >
            {events.map((event, index) => {
              const animationState = enableStaggeredAnimations
                ? itemStates[index]
                : "entered";
                ...
                <div
                  ...
                  className={cn(
                    ...
                    enableAnimations &&
                      ...
                      getAnimationClass("animate-slideOutDown")
                  )}
                  style={
                    enableAnimations && animationDelay > 0
                      ? {
                  ...
                  {enableBulkSelection && enableEventActions && (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={
                          eventActions.state.bulkSelection.isSelectAll ||
                          eventActions.state.bulkSelection.selectedIds.has(
                            event.id
                          )
                        }
                        onChange={() =>
                          eventActions.toggleEventSelection(event.id)
                        }
                        className={cn(
      ...
      {enableEventActions && eventActions.state.pendingConfirmation && (
        <ConfirmDialog
          ...
          description={
            eventActions.state.pendingConfirmation.config.description
          }
          confirmText={
            eventActions.state.pendingConfirmation.config.confirmText
          }
          ...
          isLoading={
            eventActions.state.isDeleting ||
            eventActions.state.isArchiving ||
            eventActions.state.isBulkOperating
          }
        />
      )}
    </div>

...
export function GuestOverview({
  ...
  const { data: dietaryGuests, isLoading: isDietaryLoading } = useQuery<
    Guest[]
  >({
  ...
  const plusOnesConfirmed = guests.filter(
    (g) => g.plus_one_allowed && g.plus_one_name
  ).length;
    ...
    <div className={cn("space-y-6", className)}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          subtitle={`${
            totalGuests > 0
              ? Math.round((notAttendingGuests / totalGuests) * 100)
              : 0
          }% of total`}
        />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <StatCard
          ...
          color="amber"
          subtitle={`${
            totalGuests > 0 ? Math.round((dietaryCount / totalGuests) * 100) : 0
          }% of guests`}
        />
      </div>
            ...
            {dietaryCount === 0 ? (
              ...
              <div className="space-y-3">
                <p className="text-sm text-muted-foreground mb-4">
                  {dietaryCount}{" "}
                  {dietaryCount === 1 ? "guest has" : "guests have"} dietary
                  restrictions
                </p>

...
export class ApiException extends Error {
  ...
  constructor(message: string, status: number, code?: string, details?: string | string[]) {
    super(message)
...
const retryRequest = async <T>(
  ...
  for (let attempt = 1; attempt <= config.attempts; attempt++) {
    try {
      ...
      if (error instanceof ApiException && error.status >= 400 && error.status < 500 && error.status !== 429) {
        throw error
      }
...
export const getErrorDetails = (error: unknown): string | string[] | undefined => {
  if (isApiException(error)) {
...
export const withRetry = (retries: Partial<RetryConfig> = {}): { retries: RetryConfig } => ({
...
export const legacyAuthApi = {
  ...
  confirmPasswordReset: (data: { email: string; confirmation_code: string; new_password: string }) => 

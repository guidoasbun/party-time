...
export class BudgetService {
  ...
  /**
   * Get all expenses for an event (paginated)
   */
  async getExpenses(
    eventId: UUID,
  ...
  async bulkCreateCategories(
    ...
    return api.post<
      ...
      { categories: BudgetCategoryCreate[] }
    >(`${API_ENDPOINTS.BUDGET.CATEGORIES(eventId)}/bulk`, { categories });
  }
  ...
  async bulkCreateExpenses(
    ...
    return api.post<
      ...
      { expenses: ExpenseCreate[] }
    >(`${API_ENDPOINTS.BUDGET.EXPENSES(eventId)}/bulk`, { expenses });
  }
  ...
  async bulkUpdateExpenses(
    ...
    return api.patch<
      ...
      { updates: Array<{ expense_id: UUID; data: ExpenseUpdate }> }
    >(`${API_ENDPOINTS.BUDGET.EXPENSES(eventId)}/bulk`, { updates });
  }
  ...
  async importBudgetData(
    ...
    formData.append(
      ...
      options.create_missing_categories.toString()
    );
    ...
    return api.upload<{
      ...
      expenses_created: number;
      errors: Array<{ row: number; error: string }>;
    }>(`${API_ENDPOINTS.BUDGET.SUMMARY(eventId)}/import`, file, onProgress);
  }
  ...
  getDefaultCategories(
    eventType: string
  ): Omit<BudgetCategoryCreate, "event_id">[] {
